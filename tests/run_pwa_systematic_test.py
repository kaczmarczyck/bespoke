...
async def run_systematic_test():
    ...
    try:
        ...
        print("CRITICAL: Failed to locate Chrome target page:", e)
    ...
    async with websockets.connect(ws_url) as ws:
                ...
                if cid in pending_responses:
                    ...
                    if "result" in msg and "result" in msg["result"]:
                        ...
                        if msg["result"].get("exceptionDetails"):
                            raise Exception(f"JS Exception: {res.get('description')}")
                        return res.get("value")
                    raise Exception(f"Invalid response: {msg}")
                await asyncio.sleep(0.1)
            raise Exception(f"CDP command {cid} timed out waiting for response!")
        ...
        try:
            ...
            # Find the download button for german_english
            ...
            if not has_play:
                raise Exception("German-English play button not found!")
            ...
            if not is_study_active:
                raise Exception("Study screen is not active after clicking Play!")
            ...
            if not front_text or front_text == "---":
                raise Exception("Flashcard front text did not load properly!")
            print("Step 4 Success!")

            # --- Test Step 5: Flip Card ---
            print("\n--- Test Step 5: Flipping the card ---")
            # Make sure Flip button is visible and click it
            await eval_js("document.getElementById('btn-flip').click()")
            await asyncio.sleep(0.5)  # Let transition finish
            ...
            if not is_unflipped:
                raise Exception("Card did not unflip after clicking Next Card!")

            next_front_text = await eval_js(
                "document.getElementById('card-front-text').textContent.trim()"
            )
            print(f"Next card front text: '{next_front_text}'")
            if not next_front_text or next_front_text == "---":
                raise Exception("New flashcard did not draw or load properly!")

            print("Step 6 Success!")

            # --- Test Step 7: Go back to deck list ---
            print("\n--- Test Step 7: Returning to home screen ---")
            await eval_js("document.getElementById('btn-back-to-decks').click()")
            await asyncio.sleep(1.0)
            is_home_active = await eval_js(
                "document.getElementById('screen-decks').classList.contains('active')"
            )
            if not is_home_active:
                raise Exception("Home screen not active after clicking Back!")
            ...
            if not has_chinese_download:
                raise Exception("Traditional Chinese-German download button not found!")
            ...
            if not has_chinese_play:
                raise Exception("Traditional Chinese play button not found!")
            ...
            if not ch_front_text or ch_front_text == "---":
                raise Exception("Chinese card front did not load properly!")
            ...
            if not is_flipped:
                raise Exception("Traditional Chinese card did not flip!")
            ...
            if not ch_next_front or ch_next_front == "---":
                raise Exception("Chinese next card did not draw properly!")

            print("Step 11 Success!")
            success = True

        except Exception as test_err:
            print(f"\nTEST FAILED: {test_err}")
            errors.append(str(test_err))
        finally:
    ...
    if success and not errors:
        print("\n=== SYSTEMATIC INTEGRATION TEST COMPLETED SUCCESSFULLY! ===")
        sys.exit(0)
    else:
        print("\n=== SYSTEMATIC INTEGRATION TEST FAILED! ===")
        sys.exit(1)
...
if __name__ == "__main__":
    asyncio.run(run_systematic_test())
